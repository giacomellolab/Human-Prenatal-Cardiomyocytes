...
CellphoneDB is then run and heatmap for all ligand_receptor interactions between all clusters generated in the user/out folder.
When these commands have been run in Python as below or directly from command line, the plots can be generated in R as shown in this notebook.
The resulting cellphoneDB files:pvalues.txt and means.txt are stored in the https://github.com/giacomellolab/Human-Prenatal-Cardiomyocytes/data/ for further use in this notebook. So are also count_network for outflow tract and atria, respecitvely.
```{r}
